...
public class AS1Comparator implements Comparator<Node> {
	...
	public int compare(Node s1, Node s2) {
		...
		for (Ally ally : AlliesStart) {
			if (ally.isCarried() == false && ally.isDropped() == false)
				damageStart += ally.getDamage();
		}
		for (Ally ally : AlliesDest) {
			if (ally.isCarried() == false && ally.isDropped() == false)
				damageDest += ally.getDamage();
		}
		return (s1.totalCost() + damageStart) - (s2.totalCost() + damageDest);
